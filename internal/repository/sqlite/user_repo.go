...
func (r *userRepo) GetByID(ctx context.Context, id int64) (*domain.User, error) {
	row := r.db.QueryRowContext(ctx, `
		SELECT id, username, first_name, last_name, admin_id, is_blocked, created_at
...
func scanUser(row *sql.Row) (*domain.User, error) {
	...
	err := row.Scan(&u.ID, &u.Username, &u.FirstName, &u.LastName, &u.AdminID, &ignored, &createdAt)
...
func scanUsers(rows *sql.Rows) ([]*domain.User, error) {
	...
	for rows.Next() {
		...
		var ignored int
		var createdAt string
		if err := rows.Scan(&u.ID, &u.Username, &u.FirstName, &u.LastName, &u.AdminID, &ignored, &createdAt); err != nil {
			return nil, err
		}
		u.CreatedAt, _ = time.Parse("2006-01-02 15:04:05", createdAt)
